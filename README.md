...
Spatial autocorrelation is a spatial analysis technique that determines whether an observation is positively or negatively correlated with its surroundings. This reveals patterns of distribution, whether random, clustered, or dispersed in a particular area. Global Moran's I measures these spatial associations simultaneously, based on location and value, providing a generalized inferential statistic (ESRI, 2024). Local Moran's I or LISA, compares all values and locations within the context of all neighbouring values, providing a comprehensive statistic describing 'hotspots' and outliers.(ESRI, 2024). In each of these tests we will examine how similar or dissimilar observations at a location (i) are from observations at neighbouring locations (j). This method of spatial analysis is a functional application of Toblers Law, which states that observations are not independant and will be influenced by observations at nearby locations (Miller H.J., 2004).

### Data 
The data used in this analysis is from 2016 and is sourced from the Statistics Canada Open Government Portal. It consists of a shapefile containing the census boundaries of multiple Canadian cities in a polygon format and a CSV file containing census data. Census data is widely used in spatial analysis as it provides us with a large amount of data describing population demographics, socioeconomics, and other key population factors for the regions where it is collected. In this tutorial we compare median total income with percent French knowledge speakers to explore the relationships between French culture and socioeconomic status in our study area. These relationships are important from a census context as they influence access to resources, public policy, policing, and locations of high density immigrant populations (Statistics Canada, 2014).
...
In order to calculate Moran's I, we need to define which locations fall within our neighbourhood. We'll use the Queen weighting scheme to define one neighbourhood as the eight neighbouring polygons located vertically, horizontally, and diagonally from a main point (i). Then we will use the Rook weighting scheme to define another neighbourhood as the four neighbouring polygons located vertically and horizontally from a main point (i). By selecting these neighbourhoods, we can compare how either might affect our results. From this comparison, we can decide which is most suitable for our analysis.
...
By creating a weighted neighbourhood matrix we are essentially finding out how much nearby observations deviate from the mean. There is a variety of weighted matrix possibilities including, inverse distance, nearest neighbour, and contiguity (ESRI, 2024). For this analysis we are using a basic binary contiguity weighting scheme, where each neighbour is given a weight of 1, and all other polygons are given a weight of 0.
